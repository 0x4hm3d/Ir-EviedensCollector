...
## ----------------------------------------------------------------------------------------------------------------------------------------		
## LOCATION OF TOOLS - CAN BE USB OR NETWORK DRIVE DON'T FORGET TO CREATE TOOLS FOLDER
## ----------------------------------------------------------------------------------------------------------------------------------------
...
## ----------------------------------------------------------------------------------------------------------------------------------------
	...
	Get-WMIObject Win32_NetworkAdapterConfiguration -ComputerName $target -Filter "IPEnabled='TRUE'" | select DNSHostName,ServiceName,MacAddress,@{l="IPAddress";e={$_.IPAddress -join ","}},@{l="DefaultIPGateway";e={$_.DefaultIPGateway -join ","}},DNSDomain,@{l="DNSServerSearchOrder";e={$_.DNSServerSearchOrder -join ","}},Description | Export-CSV $dest\network\netinfo.csv -NoTypeInformation | Out-Null
		...
		#Recently Transferred Files over Netbios
...
## ----------------------------------------------------------------------------------------------------------------------------------------	
		...
		#Locally/Remotely Logged on Users
...
## ----------------------------------------------------------------------------------------------------------------------------------------
		...
		#Network Adapters in Promiscuous mode
...
## ----------------------------------------------------------------------------------------------------------------------------------------
		...
		If ($OSArch -eq "64")
			...
			until ((Get-WMIobject -Class Win32_process -Filter "Name='1RawCopy64.exe'" -ComputerName $target | where {$_.Name -eq "1RawCopy64.exe"}).ProcessID -eq $null)
			}
		Else
			{
...
## ----------------------------------------------------------------------------------------------------------------------------------------
   Write-Host -Fore Magenta "Compressed Artifacts"  
     $command = '$tools\7z.exe a -tzip $dest XXXXXXXXXXXXXXXXXX'
     iex "& $command"
