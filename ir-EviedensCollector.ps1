...
##
##	Copyright 2022 !@hm3d Ahmed ElHabashi SOCL3 | IR | DFIR
...
## ----------------------------------------------------------------------------------------------------------------------------------------
		...
		$dest = "CollectionArtifacts"

## ----------------------------------------------------------------------------------------------------------------------------------------		
## LOCATION OF TOOLS - CAN BE USB OR NETWORK DRIVE DON'T FORGET TO CREATE TOOLS FOLDER IN CARBON BLACK PATH "C:\Windows\CarbonBlack\"
## ----------------------------------------------------------------------------------------------------------------------------------------
...
## ----------------------------------------------------------------------------------------------------------------------------------------
	...
	$dirList = ("$dest\autoruns","$dest\logs","$dest\network","$dest\prefetch","$dest\MicrosoftAVQuarantine","$dest\MemoryImager","$dest\AppCompat","$dest\reg","$dest\Group_Policy")
...
## Copy AMCACHE.HVE
## ----------------------------------------------------------------------------------------------------------------------------------------
				Write-Host -Fore Green "Pulling Amcache.hve file...."
...
Write-Host -Fore Green "Capturing memory"
	...
	date | select DateTime | ConvertTo-html -Body "<H2> Ram Image Started </H2>" >> $OutLevel1
		$command = '$tools\winpmem.exe $dest\MemoryImager\$CompName.raw'
...
## ----------------------------------------------------------------------------------------------------------------------------------------
	...
	##Copy FireFox History files##
		...
		$foxInet = Get-ChildItem X:\users\$user\AppData\Roaming\Mozilla\Firefox\Profiles\ -Filter "places.sqlite" -Force -Recurse | foreach {$_.Fullname}
		Foreach ($ffi in $foxInet) 
			{
			Copy-Item -Path $ffi -Destination $dest\users\$user\InternetHistory\Firefox
			}
		}
		else
		{
	...
	##Copy Microsoft Edge History files##
		$edgepath = "X:\users\$user\AppData\Local\Microsoft\Edge\User Data\Default"
		if (Test-Path -Path $edgepath) 
		{
			
			New-Item -Path $dest\users\$user\InternetHistory\Edge -ItemType Directory  | Out-Null
			$edgeInet = Get-ChildItem "X:\users\$user\AppData\Local\Microsoft\Edge\User Data\Default" -Filter "History" -Force -Recurse | foreach {$_.Fullname}
		Foreach ($edge in $edgeInet) 
			{
			Copy-Item -Path $edge -Destination $dest\users\$user\InternetHistory\Edge
			}
		}
		else
		{
...
$VERSION = (gwmi win32_OperatingSystem).Version
	$localprofiles = Get-WMIObject Win32_UserProfile -filter "Special != 'true'" -ComputerName $target | Where {$_.LocalPath -and ($_.ConvertToDateTime($_.LastUseTime)) -gt (get-date).AddDays(-15) }  #Can modify for number of days or all users
		foreach ($localprofile in $localprofiles)
		{
			...
			$dirList = ("$UserData\Recent","$UserData\Office_Recent","$UserData\Network_Recent","$UserData\temp","$UserData\Temporary_Internet_Files","$UserData\Powershell","$UserData\Cookies","$UserData\Java_Cache")
			New-Item -Path $dirList -ItemType Directory | Out-Null
		}	
		...
		# Collecting the Powershell Consoles History folder 
			#If ($Version -lt "5.4") 
			#{
			#$userpath = "C:\Documents and Settings"
			#$command = "$tools\robocopy.exe $userpath\$user\PowerShell $UserData\PowerShell /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_powershell_ConsoleHost_history.txt"
			#iex "& $command"	
			#}
		#Else 
			#{
			#$userpath = "C:\Users"
			#$command = "$tools\robocopy.exe $userpath\$user\AppData\Roaming\Microsoft\Windows\PowerShell\PSReadLine $UserData\PowerShell /ZB /copy:DAT /r:0 /ts /FP /np /E /log:$dest\users\$user\robocopy-log_powershell_ConsoleHost_history.txt"
			#iex "& $command"	
			#}
...
## ---------------------------------------------------------------------------------------------------------------------------------------- 
## Compresed Artifacts with 7z
## ----------------------------------------------------------------------------------------------------------------------------------------
   #Write-Host -Fore Green "Compresed Memory Image"  

     #$command = '$tools\7z.exe a -tzip $dest C:\Windows\CarbonBlack\Tools\CollectionArtifacts\'
     #$command = 'C:\Windows\CarbonBlack\Tools>\7z.exe a -tzip C:\Windows\CarbonBlack\Tools\Memdump.zip C:\Windows\CarbonBlack\Tools\Artifacts\MemoryImager'
     #iex "& $command"
